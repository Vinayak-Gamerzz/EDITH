...
async def generate_pdf(filename: str, content: str = "", title: str = "") -> str:
    ...
    if ("\n" in filename or len(filename) > 80) and ("\n" not in content and len(content) <= 80):
    ...
    try:
        ...
        # If substantial document, create Executive Cover Page
        raw_lines = [l.strip() for l in content.split("\n") if l.strip()]
        if len(raw_lines) >= 20:
            from reportlab.platypus import PageBreak
            story.append(Spacer(1, 1.5 * inch))
            story.append(Paragraph(doc_title, title_style))
            story.append(HRFlowable(width="100%", thickness=3, color=HexColor("#1E40AF"), spaceBefore=8, spaceAfter=14))
            
            sub_text = "Executive Briefing  •  Zenith Intelligence Engine"
            story.append(Paragraph(sub_text, h2_style))
            story.append(Spacer(1, 0.5 * inch))
            
            # Metadata block
            meta_style = ParagraphStyle(
                "CoverMeta", parent=body_style,
                fontSize=9, leading=14, textColor=HexColor("#64748B")
            )
            import datetime
            dt_str = datetime.datetime.now().strftime("%B %d, %Y")
            story.append(Paragraph(f"<b>Published:</b> {dt_str}<br/><b>Classification:</b> Executive Document<br/><b>Generated by:</b> Zenith Media Engine", meta_style))
            story.append(PageBreak())
        else:
            story.append(Paragraph(doc_title, title_style))
            story.append(HRFlowable(width="100%", thickness=2, color=HexColor("#1E40AF"), spaceBefore=2, spaceAfter=12))
        ...
        try:
            ...
            import shutil
            shutil.copy(out, gen_dir / out.name)
        except Exception:
            pass
...
async def generate_docx(filename: str, content: str = "", title: str = "") -> str:
    ...
    try:
        ...
        if len(raw_lines) >= 20:
            ...
            run_title = p_cover.add_run(doc_title)
            ...
            run_sub = p_sub.add_run("Executive Briefing  •  Zenith Intelligence Engine")
            run_sub.font.name = "Arial"
            run_sub.font.size = Pt(13)
            run_sub.font.color.rgb = RGBColor(30, 64, 175) # Sapphire
            p_sub.space_after = Pt(24)

            import datetime
            dt_str = datetime.datetime.now().strftime("%B %d, %Y")
            p_meta = doc.add_paragraph()
            p_meta.add_run(f"Published: {dt_str}\nClassification: Executive Document\nGenerated by: Zenith Media Engine")
