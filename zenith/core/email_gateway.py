...
def _is_self_sent(email_addr: str) -> bool:
    email_clean = email_addr.strip().lower()
    resend_addr = _extract_email_address(settings.resend_from) if settings.resend_from else ""
    return bool(
        (resend_addr and email_clean == resend_addr)
        or (settings.gmail_user and email_clean == settings.gmail_user.lower())
    )
...
def _is_automated_email(msg_obj, sender_email: str, subject: str = "") -> bool:
    ...
    if msg_obj.get("List-Unsubscribe") or msg_obj.get("Precedence") in ("bulk", "list", "junk"):
        return True
    if msg_obj.get("Auto-Submitted") and msg_obj.get("Auto-Submitted").strip().lower() != "no":
        return True
    subj_low = subject.lower()
    if any(k in subj_low for k in ["daily digest", "weekly digest", "evening update", "peace digest", "digest:"]):
        return True
    return False
...
async def process_incoming_emails(orpheus_instance) -> None:
    ...
    try:
        ...
        # Scan recent 15 email UIDs
        recent_ids = ids[-15:]

        for uid in recent_ids:
            try:
                # 1. Fast lightweight header peek first (under 50ms per email)
                typ, head_data = await asyncio.to_thread(
                    M.fetch, uid, "(BODY.PEEK[HEADER.FIELDS (MESSAGE-ID FROM TO SUBJECT DATE LIST-UNSUBSCRIBE PRECEDENCE AUTO-SUBMITTED)])"
                )
                if typ != "OK" or not head_data or not head_data[0]:
                    continue

                raw_headers = head_data[0][1]
                msg_obj = email_mod.message_from_bytes(raw_headers)

                msg_id = (msg_obj.get("Message-ID") or f"uid_{uid.decode()}").strip()
                sender_raw = msg_obj.get("From", "")
                subject_raw = msg_obj.get("Subject", "(no subject)").strip()
                ...
                # 3. Skip and mark self-sent emails
                if _is_self_sent(sender_email):
                    store.mark_email_processed(msg_id, sender_email, subject_raw)
                    continue

                # 4. Skip automated newsletters/marketing/digest emails unless from owner
                if not _is_owner(sender_email) and _is_automated_email(msg_obj, sender_email, subject_raw):
                    store.mark_email_processed(msg_id, sender_email, subject_raw)
                    continue

                # 5. Only fetch full RFC822 for genuinely unprocessed, non-automated emails
                typ, msg_data = await asyncio.to_thread(M.fetch, uid, "(RFC822)")
                if typ != "OK" or not msg_data or not msg_data[0]:
                    continue

                full_msg_obj = email_mod.message_from_bytes(msg_data[0][1])

                # Extract body text
                body = ""
                if full_msg_obj.is_multipart():
                    for part in full_msg_obj.walk():
                    ...
                    payload = full_msg_obj.get_payload(decode=True)
                    if payload:
                        body = payload.decode(errors="replace")
                ...
                log.exception("Error processing email UID %s: %s", uid, exc)
                try:
                    store.mark_email_processed(f"uid_{uid.decode()}", "unknown", "error")
                except Exception:
                    pass
